...
def process_file(file_path, thickness, diameter, output_dynamic_folder=None):
    ...
    # Export raw dynamic data to CSV
    if output_dynamic_folder is not None and not dyn_df.empty:
        filename_base = os.path.splitext(os.path.basename(file_path))[0]
        dynamic_csv_path = output_dynamic_folder / f'{filename_base}_dynamic.csv'
        dyn_df.to_csv(dynamic_csv_path, index=False)
        print(f"  Exported dynamic data to: {dynamic_csv_path.name}")
...
def main():
    ...
    for file_path in csv_files:
        ...
        if dimensions_df is not None and filename in dimensions_df['Filename'].values:
            ...
            print(f"  WARNING: No dimensions found for sample {filename} - skipping")
            continue
